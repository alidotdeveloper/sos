...
const Accordion = () => {
  ...
  const handleNavigate = (itemlink: string) => {
    setPath(itemlink);
    scrollToTop();
  };
        ...
        return (
          <div className="flex flex-col justify-center items-center  w-full ">
            <div className=" w-[80%]" key={index}>
              ...
              >
                <div className="flex">
                  <Typography size="lg" className="font-lexend text-white">
                    {d.title}
                  </Typography>
                </div>
              ...
              {isActive === index && (
                <div className="bg-[#F8F6F6] rounded-md  flex flex-col h-21 py-2 pb-2   ">
                  {d.navitems &&
                    Object.values(d.navitems).map(
                      (items: any, index: number) => {
                        return (
                          <Typography
                            size="lg"
                            className="font-lexend text-navy-blue p-1"
                            key={index}
                          >
                            <Link
                              onClick={() => handleNavigate(items.link)}
                              to={items.link}
                            >
                              {items.title}
                            </Link>
